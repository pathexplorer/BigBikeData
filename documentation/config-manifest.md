...
> is the machine-readable mirror of the same data.
>
> When you add or rename an env var: (1) update this file, (2) update
> `SECRET_CONFIG_MAP` in the seeder, (3) run the anti-drift verifier (§5).
> The service READMEs need **no** change.
...
|---|-------|---------|-----------|------|-------|
| 1 | **Bootstrap input** | `keys.env.{dev/prod}` (next to virtualenv, gitignored) | `main.sh` welcome/stage 0 | – | Only genuine user input + deterministic naming inputs |
| 2 | **Bootstrap output** | `names.env` (in `documentation/startup/`) | `main.sh`, `scripts/start.sh`, `scripts/configure_runtime.sh` | – | Every generated resource name and service account email, persisted once; no secret values |
...
### Generated values versus user-supplied values

The bootstrap generates deterministic infrastructure values. These values are
persisted in `names.env` and are used to prepare the Firestore and application
configuration layers. It does not generate credentials for external services.

The application itself generates three local secrets when they are absent:
`FLASK_SECRET_KEY`, `PRIVATE_UPLOAD_TOKEN`, and `DROpbox_WEBHOOK_PATH`. The
runtime configuration helper puts them into the `fullstack-app-json-keys`
payload; they are not external provider credentials.

`FRONTEND_BASE_URL` is different: it is an environment-specific user-facing
URL and cannot be generated by the GCP bootstrap. Configure the corresponding
frontend hosting domain first, for example `https://test.domen.com` for dev,
then provide that value before applying the runtime Secret Manager payload.

Use `./main.sh runtime {env}` after bootstrap to write the generated Firestore
...
### 2c. `fullstack-app-json-keys` payload = every layer-3 variable

The JSON-keys secret holds every variable whose **Layer** column includes `3` in
the registry above — that is the definition of its payload; there is no separate
list. The flat JSON keys must match exactly what `config.py` / `site_config.py`
read. The machine-readable mirror is
`SECRET_CONFIG_MAP["APP_JSON_KEYS"]` in
`gcp_actions/.../emulators/secret_manager/seed.py`.

**Re-fill it** after bootstrap (Stage 6 seeded only placeholders, which proves
IAM *access* in Stage 7, not token correctness):

```bash
# Build payload.json from keys.env.{env} / names.env (values are deterministic)
gcloud secrets versions add {org}-{env}-{app}-fullstack-app-json-keys \
    --data-file=payload.json --project={org}-{env}-{app}
```

**Firestore layer** — non-secret keys go in the doc `config/local/settings/data`
(read on every container start; cheap vs. paid Secret Manager calls):

```bash
gcloud firestore documents set config/local/settings/data --data=firestore_config.json \
    --project={org}-{env}-{app}
```

Default Firestore keys (`seed.py` `DEFAULT_CONFIG`): `CS_BUCKET_NAME`,
`EMAIL_MODE`, `GCS_PUB_INPUT_BUCKET`, `GCS_PUB_OUTPUT_BUCKET`,
`DROPBOX_TOPIC_NAME`, `LOGGING_LEVEL`. Never move these into the secret.

> **Do NOT re-seed from the old `voltaic-bridge-477610-h2` project.** Its
> Firestore doc holds *that* project's dev resource names, which do not exist in
> the new project. Populate the doc with this project's values from
> `keys.env.{env}` / `names.env`.
...
4. **Bootstrap:** `./main.sh {env}` (or `./main.sh {env} --no-welcome` once
   keys.env exists).
5. **Wire Pub/Sub after first deploy:** `./main.sh wire {env}`.
...
Configuration decays when the manifest and the machine-readable seeder
(`SECRET_CONFIG_MAP`) disagree, or when the live cloud layers drift from
`keys.env` / `names.env`. Verify the live state with:
