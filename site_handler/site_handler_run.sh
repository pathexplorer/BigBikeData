...
# Always remove the vendored copy (and the rendered hosting config, if any),
# even when the build fails or we abort. Single EXIT trap: later `trap`
# commands would replace it, so this is the only one in the script.
cleanup() {
    rm -rf ./gcp_actions
    if [ -n "${HOSTING_TMP:-}" ]; then rm -f "$HOSTING_TMP"; fi
}
trap cleanup EXIT
...
# Cleanup of HOSTING_TMP is handled by the cleanup() EXIT trap above.

# --- Post-deploy: Update Firebase Hosting ---
# Explicit --project (never rely on ambient gcloud/firebase defaults) and
# --non-interactive (CI-safe, no TTY prompts).
FIREBASE_PREVIEW_EXPIRES="${FIREBASE_PREVIEW_EXPIRES:-7d}"
if [[ "${ENV_MODE}" == "prod" ]]; then
    echo "🔥 Deploying to Firebase Hosting (production)..."
    firebase deploy --only hosting --config "${HOSTING_TMP}" --project "${GCP_PROJECT_ID}" --non-interactive
    ...
    firebase hosting:channel:deploy dev-app --config "${HOSTING_TMP}" --project "${GCP_PROJECT_ID}" --non-interactive --expires "${FIREBASE_PREVIEW_EXPIRES}"
fi